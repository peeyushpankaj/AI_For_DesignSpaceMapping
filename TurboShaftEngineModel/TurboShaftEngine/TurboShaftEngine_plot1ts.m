% Code to plot temperature-entropy diagram
%% Plot Description:
%
...
PlotTemperatureEntropyDiagram(simlog_TurboShaftEngine, [bdroot '/TurboShaftEngine/Air Properties'])
...
t = simlog.TurboShaftEngine.Ts_1.Pressure_Temperature_Sensor_G.P.series.time;
m = length(t);
T = zeros(m, 5);
s = zeros(m, 5);
for k = 1 : 5
    T(:,k) = simlog.TurboShaftEngine.(['Ts_' num2str(k)]).Pressure_Temperature_Sensor_G.T.series.values('K');
    s(:,k) = simlog.TurboShaftEngine.(['Ts_' num2str(k)]).Thermodynamic_Properties_Sensor_G.S.series.values('kJ/(kg*K)');
end
